import java.util.Scanner;

/**
 * Este programa calcula si hay probabilidad de que nuestra pareja nos está
 * siendo infiel. El programa irá haciendo preguntas que el usuario contestará
 * con verdadero o falso. Cada pregunta contestada como verdadero sumará 3
 * puntos. Las preguntas contestadas con falso no suman puntos.
 * 
 * @autor Marina Ruiz
 */

public class Ejercicio16 {
    public static void main(String args[]) {
        Scanner s = new Scanner(System.in);
        int sum = 0;
        int answer;
        String wrong = "La respuesta no es válida";
        System.out.println("Responde '0' (falso) o '1' (verdadero) a las siguientes preguntas según tu situación: ");
        System.out.print("1. Tu pareja parece estar más inquieta de lo normal sin ningún motivo aparente: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("2. Ha aumentado sus gastos de vestuario: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("3. Ha perdido el interés que mostraba anteriormente por ti: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print(
                "4. Ahora se afeita y se asea con más frecuencia (si es hombre) o ahora se arregla el pelo y se asea con más frecuencia (si es mujer): ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("5. No te deja que mires la agenda de su teléfono móvil: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("6. A veces tiene llamadas que dice no querer contestar cuando estás tú delante: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("7. Últimamente se preocupa más en cuidar la línea y/o estar bronceado/a: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("8. Muchos días viene tarde después de trabajar porque dice tener mucho más trabajo: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("9. Has notado que últimamente se perfuma más: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        System.out.print("10. Se confunde y te dice que ha estado en sitios donde no ha ido contigo: ");
        answer = s.nextInt();
        if (answer != 0 && answer != 1) {
            System.out.print(wrong);
            s.close();
            return;
        }
        sum += answer == 0 ? 0 : 3;
        s.close();

        System.out.println(sum);
        if (sum <= 10)
            System.out.print("Puntuación entre 0 y 10: ¡Enhorabuena! tu pareja parece ser totalmente fiel.");
        else if (sum <= 22)
            System.out.print(
                    "Puntuación entre 11 y 22: Quizás exista el peligro de otra persona en su vida o en su mente, aunque seguramente será algo sin importancia. No bajes la guardia.");
        else
            System.out.print(
                    "Puntuación entre 23 y 30: Tu pareja tiene todos los ingredientes para estar viviendo un romance con otra persona. Te aconsejamos que indagues un poco más y averigües que es lo que está pasando por su cabeza.");
    }
}
